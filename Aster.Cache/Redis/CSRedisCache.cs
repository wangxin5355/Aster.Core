...
namespace Aster.Cache
{
    public class CSRedisCache : IDistributedCache
    {
        ...
        public void Set(string key, byte[] value, DistributedCacheEntryOptions options)
        {
            ...
            RedisHelper.Eval(SetScript, _instance + key, new object[]
                {
        ...
        public async Task SetAsync(string key, byte[] value, DistributedCacheEntryOptions options, CancellationToken token = default(CancellationToken))
        {
            ...
            await RedisHelper.EvalAsync(SetScript, _instance + key, new object[]
                   {
                            absoluteExpiration?.Ticks ?? NotPresent,
                            options.SlidingExpiration?.Ticks ?? NotPresent,
                            GetExpirationInSeconds(creationTime, absoluteExpiration, options) ?? NotPresent,
        ...
        private byte[] GetAndRefresh(string key, bool getData)
        {
            ...
            if (getData)
            {
                var ret = RedisHelper.HMGet<byte[]>(_instance + key, AbsoluteExpirationKey, SlidingExpirationKey, DataKey);
            ...
            {
                results = RedisHelper.HMGet(_instance + key, AbsoluteExpirationKey, SlidingExpirationKey);
            }
        ...
        private async Task<byte[]> GetAndRefreshAsync(string key, bool getData, CancellationToken token = default(CancellationToken))
        {
            ...
            if (getData)
            {
                var ret = await RedisHelper.HMGetAsync<byte[]>(_instance + key, AbsoluteExpirationKey, SlidingExpirationKey, DataKey);
                results = new object[] { ret[0] == null ? null : Encoding.UTF8.GetString(ret[0]), ret[1] == null ? null : Encoding.UTF8.GetString(ret[1]), value = ret[2] };
            }
            else
            {
                results = await RedisHelper.HMGetAsync(_instance + key, AbsoluteExpirationKey, SlidingExpirationKey);
            }
        ...
        public void Remove(string key)
        {
            ...
            RedisHelper.Del(key.Split('|').Select(x => $"{_instance}{x}").ToArray());
        ...
        public async Task RemoveAsync(string key, CancellationToken token = default(CancellationToken))
        {
            ...
            await RedisHelper.DelAsync(key.Split('|').Select(x => $"{_instance}{x}").ToArray());
        ...
        private void Refresh(string key, DateTimeOffset? absExpr, TimeSpan? sldExpr)
        {
            ...
            if (sldExpr.HasValue)
            {
                ...
                RedisHelper.Expire(_instance + key, expr ?? TimeSpan.Zero);
                // TODO: Error handling
            }
        }
        ...
        private async Task RefreshAsync(string key, DateTimeOffset? absExpr, TimeSpan? sldExpr, CancellationToken token = default(CancellationToken))
        {
            ...
            if (sldExpr.HasValue)
            {
                ...
                await RedisHelper.ExpireAsync(_instance + key, expr ?? TimeSpan.Zero);
                // TODO: Error handling
            }
        }
